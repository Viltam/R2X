...
sed -i "s#/10000#$VMESS_WSPATH#g;s#/20000#$VLESS_WSPATH#g" /etc/nginx/conf.d/default.conf

URL=${REPL_SLUG}.${REPL_OWNER}.okteto.net
...
qrencode -o /etc/nginx/html/M$UUID.png $vmlink
qrencode -o /etc/nginx/html/L$UUID.png $vllink

cat > /etc/nginx/html/$UUID.html<<-EOF
<html>
<head>
<title>Replit</title>
<style type="text/css">
